	...
	DEFAULT = {
		CONFIG = {
			...
			Jumpscare = {
				...
				Background2 = Color3.fromRGB(255, 0, 0),
				Face1 = "",
				Face2 = "",
				Sound1 = "",
				Sound2 = ""
...
local function ShowJumpscare(config)
	local ScreenGui = Instance.new("ScreenGui")
	ScreenGui.Name = "CustomJumpscare"
	...
	BG.BackgroundColor3 = config.Background1 or Color3.fromRGB(255, 0, 0)
	...
	local function playSound(id, volume)
		...
		s.SoundId = typeof(id) == "number" and "rbxassetid://" .. tostring(id) or id
	...
	task.spawn(function()
		BG.BackgroundColor3 = config.Background1 or Color3.fromRGB(255, 0, 0)
		Img.Image = config.Face1 or ""
		playSound(config.Sound1, 6)
		
		task.wait(0.6)
		
		BG.BackgroundColor3 = config.Background2 or Color3.fromRGB(255, 0, 0)
		Img.Image = config.Face2 or ""
		playSound(config.Sound2, 7)
		
		local shakeCount = 0
		local shakeConnection
		shakeConnection = RunService.RenderStepped:Connect(function()
			if shakeCount < 20 then
				local offsetX = math.random(-30, 30)
				local offsetY = math.random(-30, 30)
				Img.Position = UDim2.new(0.5, offsetX, 0.5, offsetY)
		...
		local tween = TweenService:Create(BG, TweenInfo.new(0.4), {BackgroundTransparency = 1})
		local tween2 = TweenService:Create(Img, TweenInfo.new(0.4), {ImageTransparency = 1})
		tween:Play()
		tween2:Play()
    ...
    if newHealth == 0 then
        ...
        if config.Achievement.Death.Enabled then
            ...
            achievementGiver({
                ...
                Desc = config.Achievement.Death.Desc,
                Reason = config.Achievement.Death.Reason,
                Image = config.Achievement.Death.Image
            })
        end
    end
end
            ...
            if self:IsAlive() then
                ...
                if self.Config.Achievement.Despawned.Enabled then
                    ...
                    achievementGiver({
                        ...
                        Desc = self.Config.Achievement.Despawned.Desc,
                        Reason = self.Config.Achievement.Despawned.Reason,
                        Image = self.Config.Achievement.Despawned.Image
                    })
                end
            end
        end
